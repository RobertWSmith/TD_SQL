SELECT
    ...
    --CAL.MNTH AS FCDD_CMPL_MONTH,
    P.PBU ,
    SUBSTR( P.MATL_ID, 10 ) || ' - ' || P.DESCR AS "Material Description",
    -- P.OWN_CUST AS "Common Owner",

    CAST( CASE 
        WHEN CAST( P.ORD_DT_TO_FCDD_DYS / 7 AS INTEGER ) = 0
            THEN 'Same Week'
        ELSE  'Weeks After:' || CAST( ( P.ORD_DT_TO_FCDD_DYS / 7 ) AS FORMAT '-9(2)' )
    END AS VARCHAR(20) ) AS "Order Dt to FCDD Interval",
    
    SUM( CAST( P.CURR_ORD_QTY AS INTEGER ) ) AS "Total Ordered Qty",

    SUM( CAST( P.FCDD_TOTAL_HIT_QTY AS INTEGER ) ) AS "FCDD Total Hit Qty",
    SUM( CAST( P.FCDD_ON_TIME_QTY AS INTEGER ) ) AS "FCDD Total On Time Qty"
...
SELECT
    ...
    CUST.OWN_CUST_NAME,
    CUST.OWN_CUST_ID,
    CUST.OWN_CUST_NAME || ' - ' || CUST.OWN_CUST_ID AS OWN_CUST,
    CUST.CUST_GRP_ID,
    CUST.CUST_GRP2_CD,
    ...
    CASE
        WHEN CAST( POL.FRST_PROM_DELIV_DT - POL.ORDER_DT AS INTEGER ) <= 0
            THEN 0
        ELSE CAST( POL.FRST_PROM_DELIV_DT - POL.ORDER_DT AS INTEGER )
    END AS ORD_DT_TO_FCDD_DYS,
...
WHERE
    ...
    AND FCDD_CMPL_DT BETWEEN DATE '2013-01-01' AND ( CURRENT_DATE - 1 )
    AND POL.PBU_NBR IN ( '01', '03' ) 
        
    ) P

    INNER JOIN GDYR_BI_VWS.GDYR_CAL CAL
        ON CAL.DAY_DATE = P.FCDD_CMPL_DT
