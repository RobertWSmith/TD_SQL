...
SELECT
    ...
    , CASE
        WHEN EXPORT_IND = 'D'
            THEN (CASE
                ...
                WHEN CUST.OWN_CUST_ID = '00A0009802' AND MATL.PBU_NBR = '01' AND OE_REPL_IND = 'Repl' THEN 12
                WHEN CUST.OWN_CUST_ID = '00A0005041' AND MATL.PBU_NBR = '01' AND OE_REPL_IND = 'Repl' THEN 13
                WHEN CUST.OWN_CUST_ID = '00A0005037' AND MATL.PBU_NBR = '01' AND OE_REPL_IND = 'Repl' THEN 14
                WHEN CUST.OWN_CUST_ID = '00A0007200' AND MATL.PBU_NBR = '01' AND OE_REPL_IND = 'Repl' THEN 15
    ...
    --, CUST.OWN_CUST_NAME
    , CASE
        WHEN CUST.TIRE_CUST_TYP_CD = 'NA' THEN 'REPL'
...
FROM ON_TIME_DELIVERY OTD
    ...
    INNER JOIN (
            SELECT
                ...
                , Q.OE_REPL_IND
                , Q.MTH_PBU_ORDER_QTY
                , SUM(Q.MTH_PBU_ORDER_QTY) OVER (PARTITION BY Q.METRIC_TYPE, Q.PBU_NBR, Q.EXT_MATL_GRP_ID, Q.OE_REPL_IND ORDER BY Q.COMPLETE_MTH ROWS UNBOUNDED PRECEDING) AS YTD_PBU_ORDER_QTY
            FROM (
                SELECT
                    METRIC_TYPE
                    , COMPLETE_MTH
    ...
    INNER JOIN (
            SELECT
                ...
                , Q.CO_ORDER_QTY
                , Q.CO_ONTIME_QTY
                , SUM(Q.CO_ORDER_QTY) OVER (PARTITION BY Q.METRIC_TYPE, Q.OWN_CUST_RNK, Q.OWN_CUST_ID, Q.PBU_NBR, Q.EXT_MATL_GRP_ID, Q.OE_REPL_IND ORDER BY Q.COMPLETE_MTH ROWS UNBOUNDED PRECEDING) AS YTD_CO_ORDER_QTY
                , SUM(Q.CO_ONTIME_QTY) OVER (PARTITION BY Q.METRIC_TYPE, Q.OWN_CUST_RNK, Q.OWN_CUST_ID, Q.PBU_NBR, Q.EXT_MATL_GRP_ID, Q.OE_REPL_IND ORDER BY Q.COMPLETE_MTH ROWS UNBOUNDED PRECEDING) AS YTD_CO_ON_TIME_QTY
            FROM (
                SELECT
                    METRIC_TYPE
                    , COMPLETE_MTH
...
/*WHERE
    OTD.COMPLETE_MTH = DATE '2014-10-01'
