SELECT
    ...
    CASE
        WHEN POL.CMPL_DT < CURRENT_DATE AND POL.CMPL_IND = 1
            THEN 'Y'
        ELSE 'N'
    END AS FRDD_METRIC_INCLUDE_IND,
    CAST( CASE
        WHEN POL.CMPL_DT < CURRENT_DATE AND POL.CMPL_IND = 1
            THEN 'FRDD Included'
        ELSE 'FRDD Excluded'
    ...
    CASE
        WHEN POL.FPDD_CMPL_DT < CURRENT_DATE AND POL.FPDD_CMPL_IND = 1 AND POL.FRST_PROM_DELIV_DT IS NOT NULL
            THEN 'Y'
        ELSE 'N'
    END AS FCDD_METRIC_INCLUDE_IND,
    CAST( CASE
        WHEN POL.FPDD_CMPL_DT < CURRENT_DATE AND POL.FPDD_CMPL_IND = 1 AND POL.FRST_PROM_DELIV_DT IS NOT NULL
            THEN 'FCDD Included'
        ELSE 'FCDD Excluded'
    ...
    CASE
        WHEN POL.PRFCT_ORD_HIT_DESC = 'FRDD Hit - No Stock'
            THEN CAST( FRDD_CMPL_DT - CAST( ( FRDD + 1 ) AS DATE ) AS INTEGER 
    END) AS FRDD_NO_STK_CMPL_DT_DIFF,
    CASE
        WHEN POL.PRFCT_ORD_FPDD_HIT_DESC = 'FCDD Hit - No Stock'
            THEN CAST( FRDD_CMPL_DT - CAST( ( FCDD + 1 ) AS DATE ) AS INTEGER ) 
    END AS FCDD_NO_STK_CMPL_DT_DIFF,
...
WHERE
    ...
    AND ( 
        POL.CMPL_DT BETWEEN ( CAST( SUBSTR( CAST( ADD_MONTHS( CURRENT_DATE, -6 ) AS CHAR(10) ), 1, 7) || '-01' AS DATE ) ) AND ( CURRENT_DATE - 1 )
        OR POL.FPDD_CMPL_DT BETWEEN ( CAST( SUBSTR( CAST( ADD_MONTHS( CURRENT_DATE, -6 ) AS CHAR(10) ), 1, 7) || '-01' AS DATE ) ) AND ( CURRENT_DATE - 1 ) 
        )
