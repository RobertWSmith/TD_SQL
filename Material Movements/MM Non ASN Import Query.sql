SELECT
    ...
    , CAST(CASE WHEN MM.MVMNT_TYP_CD = '102' THEN 'IC' ELSE 'IP' END || MM.UNION_TYPE AS VARCHAR(25)) AS DATA_TYPE
    , MM.RPT_MTH_DT
    , MM.MATL_ID
    , MM.FACILITY_ID
    , MM.VEND_ID
...
FROM (
    ...
    FROM GDYR_BI_VWS.NAT_MATL_DOC_ITM_CURR MDI
        ...
        INNER JOIN GDYR_VWS.PRCH_DOC_ITM PDI
            ...
            AND PDI.PRCH_DOC_ITM_ID = PDI.PRCH_DOC_ITM_ID
            AND PDI.ORIG_SYS_ID = 2
            AND PDI.EXP_DT = CAST('5555-12-31' AS DATE)
            AND PDI.CO_CD IN ('N101', 'N102', 'N266')
            AND PDI.PRCH_CTGY_CD = 'F'
            AND (
                PDI.CNFRM_CNTRL_ID IS NULL
                OR PDI.CNFRM_CNTRL_ID IS NOT IN ('Z004', 'Z005')
                )
