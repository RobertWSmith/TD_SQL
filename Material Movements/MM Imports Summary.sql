...
FROM GDYR_BI_VWS.NAT_MATL_DOC_ITM_CURR MDI
    ...
    INNER JOIN GDYR_BI_VWS.TIME_PERD_DESC_SYSPR TPD
        ON MD.POST_DT BETWEEN TPD.PERD_BEGIN_DT AND TPD.PERD_END_DT
...
WHERE
    ...
    AND TPD.PERD_DESC = #prompt('P_TimePeriod', 'text')#
...
FROM GDYR_VWS.DELIV_DOC_ITM DDI
    ...
    INNER JOIN GDYR_BI_VWS.TIME_PERD_DESC_SYSPR TPD
        ON DD.ORIG_DOC_DT BETWEEN TPD.PERD_BEGIN_DT AND TPD.PERD_END_DT
...
WHERE
    ...
    AND (DDI.DELIV_DOC_ID, DDI.DELIV_DOC_ITM_ID) IN (
        ...
        FROM GDYR_BI_VWS.CHG_DOC_ITM CDI
            INNER JOIN GDYR_BI_VWS.TIME_PERD_DESC_SYSPR TPD
                ON CDI.SRC_CRT_DT BETWEEN TPD.PERD_BEGIN_DT AND TPD.PERD_END_DT
        WHERE
            CDI.ORIG_SYS_ID = 2
