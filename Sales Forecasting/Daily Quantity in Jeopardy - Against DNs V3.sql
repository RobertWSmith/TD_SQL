SELECT
    ...
    , CAST(CASE
        WHEN OPN.SNAP_SL_DELIV_BLK_CD > '' OR OPN.SNAP_HDR_DELIV_BLK_CD > ''
            THEN 99
        WHEN OPN.SHIP_COND_ID IN ('00', 'LW') AND OPN.OE_REPL_IND = 'Replacement'
            THEN 10
        WHEN OPN.CUST_GRP2_CD IN ('MAN') AND OPN.OE_REPL_IND = 'Replacement'
            THEN 5
        ELSE 0
    ...
    , NULL AS SHIP_COND_ID
    , NULL AS PROD_ALLCT_DETERM_PROC_ID
    , OPN.CUST_GRP2_CD

    --, OPN.SNAP_HDR_DELIV_BLK_CD
    --, OPN.SNAP_SL_DELIV_BLK_CD
    , CASE
        WHEN OPN.SNAP_SL_DELIV_BLK_CD > ''
            THEN OPN.SNAP_SL_DELIV_BLK_CD
        ELSE OPN.SNAP_HDR_DELIV_BLK_CD
        END AS DELIV_BLK_CD
    , NULL AS SNAP_DELIV_PRTY_ID
    , NULL AS SNAP_SPCL_PROC_ID

    , NULL AS CURR_HDR_DELIV_BLK_CD
    , NULL AS CURR_SL_DELIV_BLK_CD
    , NULL AS CURR_DELIV_PRTY_ID
    , NULL AS CURR_SPCL_PROC_ID
    ...
    , NULL AS SNAP_FACILITY_ID
    , NULL AS SNAP_FACILITY_NAME
    , NULL AS CURR_FACILITY_ID
    , OPN.REDIRECT_IND

    , NULL AS HDR_CRT_MTH_DT
    , NULL AS ITM_CRT_MTH_DT

    --, OPN.HDR_CRT_DT - EXTRACT(DAY FROM OPN.HDR_CRT_DT) +1 AS HDR_CRT_MTH_DT
    --, OPN.ITM_CRT_DT - EXTRACT(DAY FROM OPN.ITM_CRT_DT) +1 AS ITM_CRT_MTH_DT
...
GROUP BY
    ...
    --, OPN.SHIP_COND_ID
    --, OPN.PROD_ALLCT_DETERM_PROC_ID
    , OPN.CUST_GRP2_CD

    --, OPN.SNAP_HDR_DELIV_BLK_CD
    --, OPN.SNAP_SL_DELIV_BLK_CD
    , DELIV_BLK_CD
    --, OPN.SNAP_DELIV_PRTY_ID
    --, OPN.SNAP_SPCL_PROC_ID

    --, OPN.CURR_HDR_DELIV_BLK_CD
    --, OPN.CURR_SL_DELIV_BLK_CD
    --, OPN.CURR_DELIV_PRTY_ID
    --, OPN.CURR_SPCL_PROC_ID
    ...
    --, OPN.SNAP_FACILITY_ID
    --, OPN.SNAP_FACILITY_NAME
    --, OPN.CURR_FACILITY_ID
    , OPN.REDIRECT_IND
