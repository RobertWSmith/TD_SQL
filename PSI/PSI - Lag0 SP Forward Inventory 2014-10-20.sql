SELECT
    ...
    , GC.CAL_YR AS "Year"
    , GC.CAL_MTH AS "Month"
    , CASE WHEN MM.PBU_NBR= '01' THEN MH.MKT_CTGY_MKT_AREA_NAME
    ELSE
                                        MH.MKT_CTGY_MKT_GRP_NAME
    END AS "Category"
    , CASE WHEN MM.PBU_NBR = '01' THEN MH.MKT_CTGY_PROD_GRP_NAME
    ELSE
                                        MH.TIERS
    END AS "Tier"
    ...
    , MTI.GRN_TIRE_CTGY_CD
    , MTI.SAFE_STK_QTY
    , MTI.MATL_MIN_INV_RQT_QTY
    , MTI.MATL_CYCL_STK_QTY
    , MTI.MATL_MAX_INV_QTY
    , MTI.MATL_AVG_INV_QTY
    , MTI.MIN_RUN_QTY
    ...
    LEFT OUTER JOIN (
            ...
            FROM (
                ...
                SELECT
                    ...
                    , SUM(CASE
                        WHEN PPC.QUERY_TYPE = 'C'
                            THEN ZEROIFNULL(PPC.QUANTITY)
                        ELSE 0
                        END) AS CREDIT_QTY
                    , SUM(CASE
                        WHEN PPC.QUERY_TYPE = 'P'
                            THEN ZEROIFNULL(PPC.QUANTITY)
                        ELSE 0
                        END) AS PLAN_QTY
                ...
                LEFT OUTER JOIN (
                        ...
                        QUALIFY
                            FMC.LVL_DESIGN_EFF_DT = MAX(FMC.LVL_DESIGN_EFF_DT) OVER (PARTITION BY CAL.MONTH_DT, FMC.MATL_ID, FMC.FACILITY_ID, FMC.LVL_GRP_ID)
                            AND ROW_NUMBER() OVER (PARTITION BY CAL.MONTH_DT, FMC.MATL_ID, FMC.FACILITY_ID, FMC.LVL_GRP_ID ORDER BY FMC.LVL_DESIGN_EFF_DT DESC) = 1
            ...
            QUALIFY
                SF.LVL_DESIGN_EFF_DT = MAX(SF.LVL_DESIGN_EFF_DT) OVER (PARTITION BY PRD.MONTH_DT, PRD.MATL_ID, PRD.SRC_FACILITY_ID)
            ) MTI
