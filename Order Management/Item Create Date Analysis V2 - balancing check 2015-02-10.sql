SELECT
    ...
    , SUM(O.OPEN_CNFRM_QTY_CURR) AS OPEN_CNFRM_QTY_CURR
    , SUM(O.OPEN_CNFRM_QTY_FTR) AS OPEN_CNFRM_QTY_FTR
...
SELECT
    ...
    , CASE
        WHEN ORD.REJ_REAS_ID = '' 
            THEN ORD.WAIT_LIST_QTY
        ELSE 0
        END AS WAIT_LISTED_QTY
    , CASE
        WHEN ORD.REJ_REAS_ID <> ''
            THEN (CASE
        ...
        END AS CANCEL_QTY
    , CASE
        WHEN ORD.REJ_REAS_ID <> '' AND TOT_DN_QTY >= 0
            THEN TOT_DN_QTY
        ELSE 0
        END AS NET_DN_CNCL_QTY
    , CASE
        WHEN TOT_DN_QTY = ORDER_QTY OR ORDER_QTY = WAIT_LISTED_QTY
            THEN 0
        ELSE ORDER_QTY - (TOT_CONFIRMED_QTY + CANCEL_QTY)
        END AS UNCONFIRMED_QTY

    , CASE
        WHEN TOT_DN_QTY >= ORD.ORD_QTY
            THEN 0
        ELSE ORD.CNFRM_QTY_CURR - (IN_PROCESS_QTY_CURR + ACTL_GOODS_ISS_QTY)
        END AS OPEN_CNFRM_QTY_CURR
    , CASE
        WHEN TOT_DN_QTY >= ORD.ORD_QTY
            THEN 0
        ELSE ORD.CNFRM_QTY_FTR - IN_PROCESS_QTY_FTR
        END AS OPEN_CNFRM_QTY_FTR

    , ZEROIFNULL(DLV.IN_PROC_QTY_CURR) AS IN_PROCESS_QTY_CURR
    , ZEROIFNULL(DLV.IN_PROC_QTY_FTR) AS IN_PROCESS_QTY_FTR
    , ZEROIFNULL(DLV.AGID_QTY) AS ACTL_GOODS_ISS_QTY
    
    , ORDER_QTY - (CANCEL_QTY + WAIT_LISTED_QTY + UNCONFIRMED_QTY + OPEN_CNFRM_QTY_CURR + OPEN_CNFRM_QTY_FTR + IN_PROCESS_QTY_CURR + IN_PROCESS_QTY_FTR + ACTL_GOODS_ISS_QTY) AS NET_IMBALANCE_QTY

    -- RAW OR TOTAL VALUES, NOT USED FOR BALANCING
    , ORD.CNFRM_QTY_CURR
    , ORD.CNFRM_QTY_FTR
    , ORD.TOT_CNFRM_QTY AS TOT_CONFIRMED_QTY
    , TOT_CONFIRMED_QTY - TOT_DN_QTY AS TOT_OPEN_CONFIRMED_QTY
    , ZEROIFNULL(DLV.DELIV_QTY) AS TOT_DN_QTY
    , ZEROIFNULL(DLV.TOT_IN_PROC_QTY) AS TOT_IN_PROCESS_QTY

    , CASE
        WHEN TOT_DN_QTY > ORD.ORD_QTY
            THEN TOT_DN_QTY - ORD.ORD_QTY
        ELSE 0
        END AS OVERSHIP_QTY
...
FROM (
    ...
    SELECT
        ...
        , MAX(CASE WHEN OD.WAIT_LIST_CD = 'Y' THEN OD.ORDER_QTY ELSE 0 END) AS WAIT_LIST_QTY

        , SUM(OD.CNFRM_QTY) AS TOT_CNFRM_QTY
        -- PGI MONTH = ITEM CREATE MONTH
        , SUM(CASE
            WHEN PGI_CAL.MONTH_DT = ITM_CAL.MONTH_DT
                THEN OD.CNFRM_QTY
            ELSE 0
            END) AS CNFRM_QTY_CURR
        -- PGI MONTH = ITEM CREATE MONTH
        , SUM(CASE
            WHEN PGI_CAL.MONTH_DT > ITM_CAL.MONTH_DT
                THEN OD.CNFRM_QTY
            ELSE 0
            END) AS CNFRM_QTY_FTR
    ...
    WHERE
        ...
        AND (
                (OD.CUST_GRP2_CD = 'TLB' AND OD.ORDER_DT = ITM_CAL.DAY_DATE)
                OR (OD.CUST_GRP2_CD <> 'TLB' AND OD.ORDER_LN_CRT_DT = ITM_CAL.DAY_DATE)
            )
    ...
    GROUP BY
        ...
        LEFT OUTER JOIN (
            SELECT
                ...
                , SUM(CASE
                    WHEN DD.ACTL_GOODS_MVT_DT = DD.ORIG_DOC_DT
                        THEN DDI.ACTL_DELIV_QTY
                    ELSE 0
                    END) AS AGID_QTY
                , IN_PROC_QTY_CURR + IN_PROC_QTY_FTR AS TOT_IN_PROC_QTY
                , SUM(CASE
                    WHEN (DD.ACTL_GOODS_MVT_DT IS NULL OR DD.ACTL_GOODS_MVT_DT <> DD.ORIG_DOC_DT) AND DN_CAL.MONTH_DT = PGMV_CAL.MONTH_DT
                        THEN DDI.ACTL_DELIV_QTY
                    ELSE 0
