...
WHERE
    OD.FRST_PLN_GOODS_ISS_DT >= ADD_MONTHS(CURRENT_DATE - EXTRACT(DAY FROM CURRENT_DATE) + 1, 1)
    AND (OD.PLN_MATL_AVL_DT, OD.FACILITY_ID, OD.MATL_ID) IN (
...
SELECT
    ...
    , CASE
        ...
        END AS AVAIL_DT
    , CAST(CASE 
        WHEN OD.FRST_PLN_GOODS_ISS_DT >= ADD_MONTHS(CURRENT_DATE - EXTRACT(DAY FROM CURRENT_DATE) + 1, 1) AND DOC_TYPE = 'SO'
            THEN 'Y'
        ELSE 'N'
