SELECT
    Q.ITM_CRT_MONTH_DT
    , Q.REQ_DELIV_DT_TYP_DESC

/*    , Q.MATL_ID
    , Q.MATL_NO_8 || ' - ' || Q.DESCR AS MATL_DESCR
    , Q.PBU_NBR || ' - ' || Q.PBU_NAME AS PBU
    , Q.MATL_STA_ID
    , Q.MATL_STA_DT
    , Q.MKT_CTGY_MKT_AREA_NBR || ' - ' || Q.MKT_CTGY_MKT_AREA_NAME AS CATEGORY

    , Q.FACILITY_ID
    , Q.FACILITY_ID || ' - ' || Q.FACILITY_NAME AS FACILITY_DESCR*/

    , Q.OWN_CUST_ID || ' - ' || Q.OWN_CUST_NAME AS CMN_OWNER
    , Q.SALES_ORG_CD || ' - ' || Q.SALES_ORG_NAME AS SALES_ORG
    , Q.DISTR_CHAN_CD || ' - ' || Q.DISTR_CHAN_NAME AS DISTR_CHAN
    , Q.CUST_GRP_ID || ' - ' || Q.CUST_GRP_NAME AS CUST_GRP

    , Q.QTY_UNIT_MEAS_ID
    , SUM(Q.ORDER_QTY) AS ORDER_QTY
    , SUM(Q.DELIV_QTY) AS DELIV_QTY
    , SUM(Q.AGID_IN_RDD_MTH_DELIV_QTY) AS AGID_IN_RDD_MTH_DELIV_QTY
    , SUM(Q.AGID_AFTER_RDD_MTH_DELIV_QTY) AS AGID_AFTER_RDD_MTH_DELIV_QTY
...
SELECT
    ...
    , ZEROIFNULL(DLV.AGID_IN_ITM_QTY) AS AGID_IN_RDD_MTH_DELIV_QTY
    , ZEROIFNULL(DLV.AGID_AFTER_ITM_QTY) AS AGID_AFTER_RDD_MTH_DELIV_QTY
    , OD.ORDER_QTY - NET_ORDER_QTY AS CANCEL_QTY
    , CASE
        WHEN OD.REJ_REAS_ID <> ''
            THEN OD.ORDER_QTY - DELIV_QTY
        WHEN OD.ORDER_QTY < DELIV_QTY
            THEN DELIV_QTY
        ELSE OD.ORDER_QTY
