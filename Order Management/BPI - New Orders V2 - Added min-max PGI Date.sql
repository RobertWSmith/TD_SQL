...
SELECT
    ...
    , CASE
        WHEN ORD.CALC_FRDD_CPM_FLG = 'Y'
            THEN 'FRDD FPGI Current'
        ELSE 'FRDD FPGI Not Current'
        END AS FRDD_FPGI_IND
    , CASE
        WHEN (
...
FROM (
    SELECT
        ...
        , CASE
            WHEN MIN(ODC.PLN_GOODS_ISS_DT) < ADD_MONTHS((CURRENT_DATE - 1) - EXTRACT(DAY FROM CURRENT_DATE - 1) + 1, 1)
                THEN 'Min PGI Date Current'
            ELSE 'Min PGI Date Not Current'
            END AS MIN_PGI_IND
        , CASE
            WHEN MAX(ODC.PLN_GOODS_ISS_DT) < ADD_MONTHS((CURRENT_DATE - 1) - EXTRACT(DAY FROM CURRENT_DATE - 1) + 1, 1)
                THEN 'Max PGI Date Current'
            ELSE 'Max PGI Date Not Current'
            END AS MAX_PGI_IND
        , CASE
...
LEFT JOIN (
    ...
    WHERE 
        DLS.ACTL_GOODS_ISS_DT BETWEEN (CURRENT_DATE - 1) - EXTRACT(DAY FROM CURRENT_DATE - 1) - 4 AND ADD_MONTHS((CURRENT_DATE - 1) - EXTRACT(DAY FROM CURRENT_DATE - 1) + 1, 2) - 1
...
LEFT JOIN (
    ...
    WHERE 
        ODC.RO_PO_TYPE_IND = 'N'
        AND ODC.ORDER_DT = OOGC.DAY_DATE
    ...
    GROUP BY
        1, 2, 3, 4
    HAVING 
        CNFRM_TOT_QTY + UNCNFRM_TOT_QTY + BACK_ORDER_TOT_QTY + DEFER_TOT_QTY + WAIT_LIST_TOT_QTY + OTHR_OPEN_TOT_QTY <> 0
    ) OOD
