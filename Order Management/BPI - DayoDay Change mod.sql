...
FROM (
    SELECT 
        COALESCE(SHP.DAY_DATE, SHPPR.DAY_DATE, OO.DAY_DATE, OOPR.DAY_DATE, IP.DAY_DATE, IPPR.DAY_DATE) AS DAY_CLSC_DATE
        , COALESCE(SHP.ORDER_FISCAL_YR, SHPPR.ORDER_FISCAL_YR, OO.ORDER_FISCAL_YR, OOPR.ORDER_FISCAL_YR, IP.ORDER_FISCAL_YR, IPPR.ORDER_FISCAL_YR) AS ORDER_FISCAL_CLSC_YR
        , COALESCE(SHP.ORDER_ID, SHPPR.ORDER_ID, OO.ORDER_ID, OOPR.ORDER_ID, IP.ORDER_ID, IPPR.ORDER_ID) AS ORDER_CLSC_ID
        , COALESCE(SHP.ORDER_LINE_NBR, SHPPR.ORDER_LINE_NBR, OO.ORDER_LINE_NBR, OOPR.ORDER_LINE_NBR, IP.ORDER_LINE_NBR, IPPR.ORDER_LINE_NBR) AS ORDER_LINE_CLSC_NBR
        , CUST.SHIP_TO_CUST_ID
        , MATL.MATL_ID
        , COALESCE(SHP.FACILITY_ID, SHPPR.FACILITY_ID, OO.FACILITY_ID, OOPR.FACILITY_ID, IP.FACILITY_ID, IPPR.FACILITY_ID) AS FACILITY_ID
    ...
    FROM (
        SELECT CAL.DAY_DATE
            ...
            , MAX(DD.DELIV_LINE_FACILITY_ID) AS FACILITY_ID
        ...
        LEFT JOIN (
            ...
            WHERE DD2.ACTL_GOODS_ISS_DT BETWEEN (CURRENT_DATE - 1) - EXTRACT(DAY FROM CURRENT_DATE - 1) + 1 AND ADD_MONTHS((CURRENT_DATE - 1) - EXTRACT(DAY FROM CURRENT_DATE - 1) + 1, 1) - 1
        ...
        WHERE DD.ACTL_GOODS_ISS_DT BETWEEN (CURRENT_DATE - 1) - EXTRACT(DAY FROM CURRENT_DATE - 1) + 1 AND ADD_MONTHS((CURRENT_DATE - 1) - EXTRACT(DAY FROM CURRENT_DATE - 1) + 1, 1) - 1
    ...
    FULL JOIN (
        SELECT CAL.DAY_DATE + 1 DAY_DATE
            , DD.ORDER_FISCAL_YR
            , DD.ORDER_ID
            , DD.ORDER_LINE_NBR
            , MAX(DD.SHIP_TO_CUST_ID) SHIP_TO_CUST_ID
    ...
    FULL JOIN (
        SELECT CAL.DAY_DATE
            ...
            , MAX(OD.FACILITY_ID) AS FACILITY_ID
        ...
        WHERE CAL.DAY_DATE BETWEEN (CURRENT_DATE - 1) - EXTRACT(DAY FROM CURRENT_DATE - 1) + 1 AND CURRENT_DATE - 1
    ...
    FULL JOIN (
        ...
        INNER JOIN NA_BI_VWS.ORDER_DETAIL OD
            ON OSL.ORDER_FISCAL_YR = OD.ORDER_FISCAL_YR
                ...
                AND OSL.SCHED_LINE_NBR = OD.SCHED_LINE_NBR
                AND CAL.DAY_DATE BETWEEN OD.EFF_DT AND OD.EXP_DT
        WHERE CAL.DAY_DATE BETWEEN (CURRENT_DATE - 1) - EXTRACT(DAY FROM CURRENT_DATE - 1) AND CURRENT_DATE - 2
            AND OD.RO_PO_TYPE_IND = 'N'
        GROUP BY 1
            , 2
            , 3
    ...
    GROUP BY 
        COALESCE(SHP.DAY_DATE, SHPPR.DAY_DATE, OO.DAY_DATE, OOPR.DAY_DATE, IP.DAY_DATE, IPPR.DAY_DATE)
        , COALESCE(SHP.ORDER_FISCAL_YR, SHPPR.ORDER_FISCAL_YR, OO.ORDER_FISCAL_YR, OOPR.ORDER_FISCAL_YR, IP.ORDER_FISCAL_YR, IPPR.ORDER_FISCAL_YR)
        , COALESCE(SHP.ORDER_ID, SHPPR.ORDER_ID, OO.ORDER_ID, OOPR.ORDER_ID, IP.ORDER_ID, IPPR.ORDER_ID)
        , COALESCE(SHP.ORDER_LINE_NBR, SHPPR.ORDER_LINE_NBR, OO.ORDER_LINE_NBR, OOPR.ORDER_LINE_NBR, IP.ORDER_LINE_NBR, IPPR.ORDER_LINE_NBR)
        , CUST.SHIP_TO_CUST_ID
        , MATL.MATL_ID
        , COALESCE(SHP.FACILITY_ID, SHPPR.FACILITY_ID, OO.FACILITY_ID, OOPR.FACILITY_ID, IP.FACILITY_ID, IPPR.FACILITY_ID)
    ) OS
GROUP BY 
    DAY_CLSC_DATE
    , SHIP_TO_CUST_ID
