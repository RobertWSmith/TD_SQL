SELECT
    ...
    , CASE
        WHEN (
            ...
            THEN 'Replacement'
        ELSE 'OE'
        END AS "OE Replacement Ind"
    , SUM(ORD.RPT_ORDER_QTY) "Order Qty"
    , SUM(CASE
            WHEN ORD.CALC_FRDD_CPM_FLG = 'N'
                THEN ORD.RPT_ORDER_QTY
            ELSE 0
            END) "Future Req Order Qty"
    , SUM(CASE
            WHEN ORD.CALC_FRDD_CPM_FLG = 'Y'
                THEN ORD.RPT_ORDER_QTY
            ELSE 0
            END) "Current Req Order Qty"
    , ZEROIFNULL(SUM(DLV.RPT_DELIV_QTY)) "Shipped Current Qty"
    , ZEROIFNULL(SUM(DIP.IN_PROC_PRI_CUR_MTH_QTY)) "In Process Current Qty"
    , ZEROIFNULL(SUM(OOD.CNFRM_PRI_CUR_MTH_QTY)) "Confirmed Current Qty"
    , SUM(CASE
            WHEN ORD.CALC_FRDD_CPM_FLG = 'Y'
                AND ORD.REJ_REAS_ID <> ' '
...
SELECT 
    CAST('DD' AS CHAR(2)) AS VERSION_CD
    , ORD.DAY_DATE
    , CUST.SALES_ORG_CD
    , CUST.DISTR_CHAN_CD
    , CUST.CUST_GRP_ID
    ...
    , SUM(CASE
            WHEN ORD.CALC_FRDD_CPM_FLG = 'Y'
                AND ORD.REJ_REAS_ID <> ' '
                AND ORD.RPT_ORDER_QTY - ZEROIFNULL(DLV.RPT_DELIV_QTY) >= 0
                THEN (ORD.RPT_ORDER_QTY - ZEROIFNULL(DLV.RPT_DELIV_QTY)) * (- 1)
            ELSE 0
            END) CANCEL_CURR_QTY
    , SUM(CASE
            WHEN ORD.CALC_FRDD_CPM_FLG = 'Y'
...
FROM (
    SELECT ODGC.DAY_DATE
        ...
        , MAX(ODC.MATL_ID) MATL_ID
        , MAX(ODC.FACILITY_ID) FACILITY_ID
        , MAX(ODC.FRST_RDD) FRST_RDD_DT
        , CASE
            WHEN FRST_RDD_DT < ADD_MONTHS((CURRENT_DATE - 1) - EXTRACT(DAY FROM CURRENT_DATE - 1) + 1, 1)
                THEN 'Y'
            ELSE 'N'
            END CALC_FRDD_CPM_FLG
        , MAX(ODC.REJ_REAS_ID) REJ_REAS_ID
        , MAX(ODC.ORDER_TYPE_ID) ORDER_TYPE_ID
        , SUM(ODC.RPT_ORDER_QTY) RPT_ORDER_QTY
    FROM NA_BI_VWS.ORDER_DETAIL ODC
    INNER JOIN GDYR_BI_VWS.GDYR_CAL ODGC
