-- SQL last updated 2014-03-12
-- Fountain Tire report to aggregate Orders & Delivered Qty
-- Report is intended to aggregate Month to Date sums of Orders and Deliveries
-- Report is scheduled once weekly and results are saved in Cognos
    ...
    SUM( CASE
        ...
        ELSE ( CASE
            WHEN DL.DELIV_DT IS NULL
                THEN OL.ORDER_QTY -- VARIANT OF FRDD LOGIC -- DON'T ZERO OUT OPEN ORDERS
            ELSE ( CASE
...
WHERE
    OL.ORDER_MONTH >= CAST( SUBSTR( CAST( ADD_MONTHS(CURRENT_DATE, -6) AS CHAR(10) ),1, 7 ) ||'-01' AS DATE )
